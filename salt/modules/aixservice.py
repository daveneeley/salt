...
def __virtual__():
    ...
    if __grains__['os'] == 'AIX':
        return __virtualname__
    return (False, 'The aix service execution module cannot be loaded: only available on AIX systems.')
...
def get_enabled():
    ...
    for svc in get_all():
        if svc in ret:
            continue
        if not os.path.exists('/etc/rc.d.d/{0}'.format(svc)):
            continue
        if enabled(svc):
            ret.append(svc)
...
def enable(name, **kwargs):
    ...
    config : /etc/rc.d
        ...
        empty string, then /etc/rc.d.d/<service> used.
        See man rc.d(5) for details.
